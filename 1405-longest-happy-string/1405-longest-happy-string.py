...
class Solution:
    ...
    def addToResult(self, result, heap_ele, max_insertions = 2):
        heap_cnt, heap_val = heap_ele
        heap_cnt = abs(heap_cnt)

        i = 0
        while heap_cnt and i < max_insertions:
            ...
            if len(result):
                if result[-1] == top_val:
                    if not len(heap):
                        ...
                        max_insertions = 2
                        if top_cnt > second_top_cnt:
                            max_insertions = 1
                        
                        
                        second_top_cnt = self.addToResult(result, second_top, max_insertions)
